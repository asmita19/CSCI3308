...
function viewStudentStats(id, toggle){
    var nag = document.getElementById(id);
    if(toggle === 0){
        nag.style.visibility = "hidden";
        nag.style.height = "0";
    }else if(toggle === 1){
        nag.style.visibility = "visible";
        nag.style.height = "suto";
    }
}
...
function loadStatsPage(){
    ...
    var loss = 0;
    var wins = 0;
    for(rows_counter = 2; rows_counter < table.rows.length; rows_counter++){
        if(parseInt(table.rows[rows_counter].cells[2].innerText) >= parseInt(table.rows[rows_counter].cells[3].innerText)){
            table.rows[rows_counter].cells[4].innerHTML = "CU BOULDER";
            wins++;
        }else{
            table.rows[rows_counter].cells[4].innerHTML = table.rows[rows_counter].cells[1].innerText;
            loss++;
        }
        document.getElementById("wins").innerText = wins.toString();
        document.getElementById("losses").innerText = loss.toString();
    }
}
...
*/
    ...
    function loadPlayersPage(){
        let sele = document.getElementById("player_selector");
        for(let i = 0; i , players.length; i++){
            let name = players[i].name;
            sele.innerHTML += "<a class=\"dropdown-item\"href=\"#\" onclick=\"switchPlayers("+i+")\">" + name + "</a>"; 
        }
    }
